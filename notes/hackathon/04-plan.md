...
- **Config:** `CLEARANCE_MM` is now 5.0 as section 15 of the spec says (it was 3.0 in code). Constants for the trigger live at the end of `config.py`: `HAND_COLOR_AREA_MM2 = 5000`, `STILL_WINDOW_S = 1.5`, `TRIGGER_GRACE_S = 0.4`.
- **Full suite:** `.venv/bin/python -m pytest -q` → `115 passed, 17 warnings in 17.96s`.
