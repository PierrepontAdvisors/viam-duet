...
- **Page, morning of day 2:** picture controls (brightness, contrast, exposure) for the live board and the photos, every robot stroke kept so the finished piece is a full vector, Ink only with colors and widths, Export SVG. On `feat/duet-page`, to merge.
- **Trigger, day 2 at the table:** the automatic trigger fired on an empty board every few seconds, so it was removed; the visitor presses Go on the page to end the turn. Morning recalibration: board had shifted about 5 mm; marks re-recorded from the look pose and the fit refreshed.
- **Abstract mode (day 2):** Haring's triple-pass lines and ticks were too thick and too literal; new default artist `abstract` draws Claude's shapes once, spaced 8 mm apart, clearance 8 mm. Pen offset retested at 2.5 mm.
- **Artists and picker (day 2 afternoon):** eight artists (Abstract, Mimic, Haring, Mondrian, Van Gogh, Architect, Designer, Shader), chosen per turn from a chip beside Go; the artist is fixed when the mark is captured and rides on the turn's messages. Mimic and Shader work from the visitor's ink with no Claude call (Shader keeps 4 mm from ink, dots denser away from the light dial); Architect squares lines, Designer rounds corners. Spec `docs/superpowers/specs/2026-09-19-duet-artists-design.md`. Suite: 153 backend, 91 page tests.
