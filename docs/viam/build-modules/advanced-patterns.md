...
Define new resource APIs, deploy custom components as remote parts, and package modules with Docker.
> Source: https://docs.viam.com/build-modules/advanced-patterns/
...
<ol>
<li>Code a new model of a built-in resource type by creating a new interface that implements the required methods from its [API definition](/reference/apis/).</li>
<li>Register the custom component on a new gRPC server instance and start the server.</li>
<li>Add the server as a [remote part](/hardware/multi-machine/add-a-remote-part/) of your machine.</li>
<li>(Optional) Ensure the remote server automatically starts when the machine boots.</li>
</ol>
...
<ol>
<li>Code a new model of a built-in resource type by subclassing it (for example, `sensor` or `arm`). Implement any required methods from its [API definition](/reference/apis/).</li>
<li>Register the custom component on a new gRPC server instance using the [`viam.rpc` library](https://python.viam.dev/autoapi/viam/rpc/index.html).</li>
<li>Add the server as a [remote part](/hardware/multi-machine/add-a-remote-part/) of your machine.</li>
<li>(Optional) Ensure the remote server automatically starts when the machine boots.</li>
</ol>
Each remote server can host one or many custom components.
...
> 
> <ul>
> <li>In Python, raise `NotImplementedError()` or use `pass` in methods you do not want to implement.</li>
> <li>In Go, return `errUnimplemented`.</li>
> </ul>
