...
> Source: https://docs.viam.com/monitor/common-errors/
...
**Description:** When following our [installation guides](/set-up-a-machine/first-machine/), you will likely encounter this message the first time you try to make an `ssh` connection to your newly-imaged [board](/components/board/)
.
...
**Description:** Your computer is not able to connect to the [board](/components/board/)
 through `ssh`.
...
**Description:** Your computer is not able to connect to the [board](/components/board/)
 through `ssh` before reaching a timeout.
...
> **Important:**
> 
> The AppImage requires FUSE version 2 (`libfuse2`), _ FUSE version 3 (`fuse3` or `libfuse3`) or versions of FUSE previous to FUSE version 2 (`fuse`).
...
**Description**: When configuring a Linux [board](/components/board/)
, Linux installations with broken or misconfigured sound libraries may experience one or both of these errors, even if not using audio components in the machine configuration.
