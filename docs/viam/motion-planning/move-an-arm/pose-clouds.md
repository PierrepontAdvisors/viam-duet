# Relax a goal with a pose cloud

Give the planner a region of acceptable destinations instead of a single exact pose, so it can find a solution faster.
> Source: https://docs.viam.com/motion-planning/move-an-arm/pose-clouds/
...
<img src="/motion-planning/move-an-arm/pose-cloud-cup.svg" alt="A cobot arm holds a cup of water above a table. A translucent dashed region on the table marks the pose cloud, with several faded ghost cups inside it showing acceptable destinations. The cup may land anywhere in the region at any rotation while staying upright." class="aligncenter" id="" style="max-width:760px" loading="lazy">
...
<img src="/motion-planning/move-an-arm/pose-cloud-target-frame.svg" alt="A tilted rectangular surface with the target frame at its center: the x and y axes lie in the surface and the z axis points along the surface normal. A dashed blue outline on the surface marks the pose-cloud region, which follows the tilt. A purple arrow offset from the z axis, with a dotted arc around it, shows the OX and OY leeway on the pointing direction and the Theta rotation about it." class="aligncenter" id="" style="max-width:820px" loading="lazy">
