...
Complete command reference for the Viam CLI: every command, subcommand, flag, and alias.
> Source: https://docs.viam.com/cli/reference/
...
| `--resource-subtype` | Resource [API namespace triplet](/reference/glossary/#term-api-namespace-triplet)
. | **Required** |
...
The `datapipelines` command provides access to data pipelines for processing machine data with [MQL](/reference/glossary/#term-mql)
 queries.
...
You specify the resource method with `--method`. Use either the short name (for example, `DoCommand` or `GetReadings`) together with `--component=<name>`, or the full [protobuf](/reference/glossary/#term-protobuf)
 package and method path (for example, `viam.service.vision.v1.VisionService.GetClassificationsFromCamera`).
...
The `module` command allows to you to work with [modules](/reference/glossary/#term-module)
.
This includes:

- Generating stub files for a new module
- Creating metadata for a [modular resource](/reference/glossary/#term-resource)
