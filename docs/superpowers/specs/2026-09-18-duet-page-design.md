...
4. **Pen down.** The "Go, robot!" button on the stage and "Pass turn" in the panel both send `{type: "pass"}`. The stage button shows only when `state.handoff` is `held` and the state is `human_turn`; the panel's Pass turn is always available, because it is the operator's escape in both handoff modes (dock mode cannot end a turn by itself until the marker dots are calibrated). The backend still refuses while a hand is over the board; the page shows its `error` text in the panel.
...
- **Session**: Short | Medium | Long · exchanges − n + · Held | Dock · artist buttons (greyed unless `state.artists` lists them; all three once the Mondrian and Van Gogh grammars are in) · Pause / Resume · Pass turn · Clear arm error (red) · light dial · energy slider.
...
- **The Go button is the trigger** (2026-09-19, at the table). The automatic trigger (stillness, hand gone, marker dots) was removed from the session loop; the human turn ends only when someone presses "Go, robot!" on the stage or Pass turn in the panel, in either handoff mode. The stage button now shows in every human turn.
- **Reset arm** (2026-09-19 afternoon). A red button beside Clear arm error sends `reset_arm`: the session stops whatever the arm is doing, clears its error, lifts the tool if it was left low, returns to the look pose, and hands the turn back to the visitor with the exchange count kept. The operator's hard reset to the observe pose after a fault or a connection drop.
...
- **Diagnostics drawer** (2026-09-19 evening). A gear-icon corner button with a socket status light opens a drawer showing everything the page streams as a timeline and a log; see `2026-09-19-duet-diagnostics-drawer-design.md`.
