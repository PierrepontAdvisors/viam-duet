...
- **Day 2, at the table:** the automatic turn trigger (section 6's stillness, hand, and dot rules, `trigger.py`) was removed after false captures on the venue floor; the human turn ends only when the visitor presses Go on the page (the `pass` command). The hand guard before capture and before every arm move stays.
- **Day 2, abstract mode:** a fourth artist, `styles/abstract.py`, is the demo default (`ARTIST = "abstract"`): Claude proposes a few abstract shapes that complement the person's marks without copying them, each drawn once as a single line, no passes or ticks; strokes closer than 8 mm to an earlier robot stroke of the same turn are dropped, and the ink clearance is 8 mm. The fallback is one calm arc beside the mark. Haring's tripled lines and dense ticks read as too heavy and too literal at the table.
