...
## 11. Follow-ups after the first walk (2026-09-20, night)

Three corrections from Nicholas after watching the built demo.

**Only the pen draws.** While the hand draws, the finger is not shown at all: the element shows a red marker on its own, leaning down and to the right from its tip, and the tip is the hotspot the pen's point is mapped to. The finger returns when the drawing ends and the hand reaches for the buttons. The Visitor pill stays with both. In the markup the hand element holds two SVGs, `svg.finger` and `svg.marker`; `.hand.draw` hides the finger and shows the marker.

**A quarter slower.** Every hand timing is 1.25 times what §3 set: glide 625 ms, hover 438, press 250, hold 625, roll glide 150, roll hover 125; the trace runs at 56 mm per second within 1.5 and 5 s; the hand's glide transition is .625 s. The robot's pace and the replay's other waits do not change. `PACE.human` rises to 16 s so the longest turn, a Shader pick after a long mark, still fits under the ceiling.

**Arrow keys seek the replay.** In replay mode the arrow keys no longer page through the photos while the recording runs on underneath, which left the state chip, the counter, the hand, and the clock out of step with the picture. They move the recording itself by one half of an exchange: the visitor's half (from the human turn) or the robot's half (from the capture), with the finish as the last stop; Right goes forward, Left back, both clamped at the ends. The schedule marks each half with a `{ mark: { turn, who } }` step; the Player tracks the half it is in and `seek(delta)` restarts the run at the target mark under a new session id, feeding every message before it at once (no waits, no cues) so the page rebuilds the strokes, the photos, and the counter, then plays on from there, so the hand, the clock, the pens, and the chips all start that half together. The page cancels the hand and stops both pens and the clock before a seek. Seeking is ignored on the welcome, before Start; after the end it starts the recording again at the chosen half. The live page's arrows still page through the photos.

**The robot's arm, and Human.** From the second walk: the robot's pen point is marked by a green robot arm in the marker's style, an element `#arm` with `svg viewBox="0 0 80 80"`: a green marker (barrel `--green`, ink tip cone at the hotspot, the cone's point at `(10, 76)` so the element is offset by `translate(-.875cqw, -6.65cqw)`) leaning up and to the right (`rotate(35 10 76)`), held by an ink-outlined paper gripper on a link with a green joint. The "Robot" pill sits inside it, so the separate `#pen-tag` goes and `placeArm` replaces `placeTag` on the robot pen's `onMove`. The hand's pill reads "Human", and so does the clock's label for the visitor's half; the cue and the `data-who` value stay `visitor`. While the hand draws it has no glide transition, so the marker sits on the trace's point every frame. The demo's defaults follow a third screenshot: full view, every overlay layer off but the bubble and the chips (the plan layer included, since the arm now shows where the robot draws and the photos carry the strokes), ink `#37e65b` at 20, strokes `#278275` at 6, the same levels, sound on.

**The hand always travels to Go (2026-09-21).** On Go-only turns the hand used to click Go from the end of the mark: the trace's last animation frame lands a few milliseconds after the drawing step's timer, and its point callback dragged the hand back to the board after the glide toward Go had begun; pick turns hid it because the roll moved the hand again. The hand now ignores the pen's points once its drawing step is over, so every turn shows the glide to Go, the lift, the squash, and the click. Assets at `?v=ds10`.
