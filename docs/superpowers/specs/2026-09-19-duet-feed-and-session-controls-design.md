...
## 10. Relaunch run (afternoon, after the merge)

The live run is kept up by `demo.sh`, which starts `duet.run` again five seconds after it exits. So the page can restart the run without touching the process itself: a `relaunch` command, refused unless the session is in `idle`, `human_turn`, `finished` or `paused` (the arm is at the look pose or stopped), tells every page "relaunching the run; the page comes back on its own in about 15 seconds" and sets uvicorn's `should_exit`; the existing shutdown stops the arm, the process exits, and `demo.sh` relaunches it with whatever code is on disk. A red Relaunch run button sits in the Run row; the page remembers the press and reloads itself the first time its socket reconnects, so new page files are always fetched. A run started without `demo.sh` refuses the command (no relauncher was given to `make_app`), and the replay harness answers it with a refusal, since it has no run.
