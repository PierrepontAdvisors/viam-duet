...
Run: `node --test 'pagetests/*.test.mjs' && $PY pagetests/replay_smoke.py`
...
### Task 13 (added 2026-09-19): picture controls, kept robot strokes, stroke widths, SVG export

Done directly after the first demo run; see spec section 14. Files: `duet/static/js/picture.js` with `pagetests/picture.test.mjs` (levels math and the SVG document, 6 tests), a Picture row and width sliders and an Export SVG button in `index.html`, the `l-done` group and z-indexes in `duet.css`, `Viewer.setDone`, `archivePlan` and `startSession` in `app.js`, and the slider, width, and export wiring in `ui.js`. Verified against `python -m duet.run --fake --exchanges 3` on port 8001: after `finished` the vector layer holds 18 robot strokes and the person's ink; brightness moves the filter intercept; Ink only shows white paper with recolored, rewidened strokes.
