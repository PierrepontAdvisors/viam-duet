...
- Work in the linked worktree: `cd "<repo>/.worktrees/duet-page/code/hackathon"`. Branch `feat/duet-page`. Never edit `duet/*.py`, `tests/`, or `config.py`: the overnight backend session owns them.
- Python commands use the main checkout's venv: `PY=<repo>/code/hackathon/.venv/bin/python`.
- Session data for the harness is gitignored and lives in the main checkout: `SESS=<repo>/code/hackathon/sessions/20260918-190258`, calibration frame `CAPS=<repo>/code/hackathon/captures`.
...
The replay harness serves the page with tonight's session over the real protocol:
    ...
    PY=<repo>/code/hackathon/.venv/bin/python
...
MAIN = Path(os.environ.get("DUET_MAIN", "<repo>/code/hackathon"))
