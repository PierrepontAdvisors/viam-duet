...
| `duet/` | The Duet drawing robot. `python -m duet.<module>`; each module's docstring has its usage. Day 1: `teach`, `stroke_bench`, `dock_test`, `aim`, `calibrate`, `claude_turn`, `turn` (terminal loop). Night 1: `session`, `recorder`, `web`, `run`, `fakes` |
| `duet/run.py` | `python -m duet.run` starts everything against the machine with the page on http://localhost:8000; `--fake` replays a recorded session through a fake camera and arm, by default the one that ships in `site/demo/sessions`; `--fake --claude` adds the real Claude call |
