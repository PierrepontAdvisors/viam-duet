...
test('assets are versioned ds10 so open pages fetch the new scripts', () => {
  const h = read('index.html');
  assert.ok(!h.includes('?v=ds5') && !h.includes('?v=ds6'), 'no older version left');
  assert.match(h, /src="\/static\/js\/app\.js\?v=ds10"/);
});
...
test('app.js records every frame before dispatch, both socket events, and every sent command', () => {
  ...
  assert.match(js, /import \{ append, frameEntry, sentEntry, socketEntry \} from '\.\/diag\.js\?v=ds10';/);
  assert.match(js, /import \{ initDiagView \} from '\.\/diagview\.js\?v=ds10';/);
  assert.match(js, /import \{ initUI, defaultsFor \} from '\.\/ui\.js\?v=ds10';/);
